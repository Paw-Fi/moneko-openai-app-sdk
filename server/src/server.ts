...
import { ASSET_BASE_URL_PLACEHOLDER, registerWidgets, type WidgetMetadata, type WidgetUris } from './widgets/register.js';
...
function hasActiveSubscription(plan: string, status: string): boolean {
  ...
  if (p === 'lifetime') return s === 'active' || s === 'trialing' || s === 'none' || s === 'paid';
...
function getConversationKey(headers: Record<string, string | undefined>): string {
  return (
...
function inferSupabaseUrlFromEdgeBaseUrl(): string | undefined {
  ...
  const match = /^(https?:\/\/[^/]+)\/functions\/v1(?:\/.*)?$/i.exec(edgeBaseUrl);
  return match?.[1];
}
...
function getAuthSession(headers: Record<string, string | undefined>): { accessToken: string; userId: string } | null {
  const key = getConversationKey(headers);
...
function verifySupabaseJwtHs256(token: string, secret: string): { userId: string } | null {
  const parts = token.split('.');
  if (parts.length !== 3) return null;
...
function getBearerToken(headers: Record<string, string | undefined>): string | undefined {
  const value = (headers['authorization'] ?? headers['Authorization'] ?? '').trim();
  if (!value) return undefined;
  const match = /^Bearer\s+(.+)$/i.exec(value);
  return match?.[1]?.trim() || undefined;
}

function tryGetSupabaseUserId(headers: Record<string, string | undefined>): string | undefined {
...
function getActorKey(headers: Record<string, string | undefined>): string {
  return tryGetSupabaseUserId(headers) ?? getConversationKey(headers);
}
...
function listCategoriesFor(headers: Record<string, string | undefined>): string[] {
  const key = getActorKey(headers);
  const custom = Array.from(customCategoriesByActor.get(key) ?? []);
  const merged = new Set<string>([...DEFAULT_CATEGORIES, ...custom].map(normalizeCategoryName).filter(Boolean));
  return Array.from(merged).sort((a, b) => a.localeCompare(b));
}

function addCategoryFor(headers: Record<string, string | undefined>, name: string): string[] {
  const key = getActorKey(headers);
...
function pickHeader(headers: Record<string, unknown>, key: string): string | undefined {
  const value =
...
function resolveAssetBaseUrl(headers: Record<string, unknown>): string {
  ...
  const host = pickHeader(headers, 'x-forwarded-host') ?? pickHeader(headers, 'host');
  if (!host) {
  ...
  const protoFromProxy = pickHeader(headers, 'x-forwarded-proto');
  const isLocalHost =
...
function resolveWidgetDomain(headers: Record<string, unknown>): string {
  const env = String(process.env.WIDGET_DOMAIN || '').trim() || String(process.env.BASE_URL || '').trim();
...
function widgetMeta(headers: Record<string, unknown>, templateUri: string): Record<string, unknown> {
  const widgetDomain = resolveWidgetDomain(headers);
  return {
  ...
  try {
    ...
    server.setRequestHandler(
      ...
      async (request: ListResourcesRequest) => {
        const headers = ((request as any).meta?.headers ?? {}) as Record<string, unknown>;
        return {
    ...
    server.setRequestHandler(
      ...
      async (request: ReadResourceRequest) => {
        ...
        const html = widget.html.replaceAll(ASSET_BASE_URL_PLACEHOLDER, assetBaseUrl);
  ...
  const mvpTools = [
    {
      ...
      description:
        'Use this when the user needs to connect their Moneko account inside this chat. Opens an embedded sign-in/register widget. Do not use for normal budgeting actions.',
      inputSchema: {
        type: 'object',
        properties: {},
      ...
      _meta: {
        'openai/outputTemplate': uris.auth,
        'openai/widgetAccessible': true,
        'openai/resultCanProduceWidget': true,
        'openai/toolInvocation/invoking': 'Opening sign-in…',
    ...
    },
    {
      ...
      description: 'Internal: stores a Supabase access token for this ChatGPT conversation.',
      inputSchema: {
        type: 'object',
        properties: {
          access_token: { type: 'string', description: 'Supabase access token (JWT).' },
        },
    ...
    },
    {
      ...
      title: 'Auth Status',
      description: 'Check whether you are signed in to Moneko for this ChatGPT conversation.',
      inputSchema: {
        type: 'object',
        properties: {},
        additionalProperties: false,
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          tab: {
            type: 'string',
            enum: ['overview', 'transactions', 'categories', 'budget', 'insights', 'membership'],
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          plan: { type: 'string', enum: ['plus', 'lifetime'], description: 'Selected plan.' },
          billingInterval: {
            type: 'string',
            enum: ['monthly', 'yearly'],
            description: 'Billing interval (required for plus; ignored for lifetime).',
          },
        },
        required: ['plan'],
        additionalProperties: false,
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          returnUrl: { type: 'string', description: 'Optional return URL on moneko.io after managing billing.' },
        },
        additionalProperties: false,
      },
      annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
    },
    {
      ...
      description:
        'Use this when the user wants to log spending or income (e.g., “I spent $12 on lunch”, “Got paid 2500”). Do not use for summaries or charts.',
      inputSchema: {
        type: 'object',
        properties: {
          type: { type: 'string', enum: ['expense', 'income'], description: 'Transaction type. Default to expense if unclear.' },
          amount: { type: 'number', description: 'Amount in major units (e.g., 12.50).' },
          category: { type: 'string', description: 'Category name (e.g., Food & Drink, Groceries, Rent).' },
          description: { type: 'string', description: 'Optional note/merchant/description (e.g., “Starbucks latte”).' },
          date: { type: 'string', description: 'Optional date in YYYY-MM-DD. If omitted, use today.' },
          currency: { type: 'string', description: 'Optional ISO currency code (e.g., USD, EUR). If omitted, use preferred currency.' },
          household_id: { type: 'string', description: 'Optional household ID (advanced).' },
          household_name: { type: 'string', description: 'Optional household name (advanced).' },
          is_recurring: { type: 'boolean', description: 'Set true if recurring (e.g., monthly).' },
          frequency: { type: 'string', description: 'Recurring frequency hint (e.g., monthly, weekly, yearly).' },
        },
    ...
    },
    {
      ...
      description:
        'Use this when the user wants to edit a previously logged EXPENSE (amount/category/date/description). Do not use to add a new transaction.',
      inputSchema: {
        type: 'object',
        properties: {
          expense_id: { type: 'string', description: 'Internal expense ID to update (do not ask user for it unless they already have it).' },
          amount: { type: 'number', description: 'New amount in major units (e.g., 9.99).' },
          category: { type: 'string' },
          description: { type: 'string', description: 'New note/description.' },
          date: { type: 'string', description: 'New date in YYYY-MM-DD.' },
          currency: { type: 'string', description: 'New ISO currency code (e.g., USD).' },
    ...
    },
    {
      ...
      description:
        'Use this when the user explicitly asks to remove a previously logged expense. Destructive: confirm intent and never delete without a clear request.',
      inputSchema: {
        type: 'object',
        properties: {
          expense_id: { type: 'string', description: 'Internal expense ID to delete.' },
        },
        required: ['expense_id'],
        additionalProperties: false,
      },
    ...
    },
    {
      ...
      title: 'Log Expense',
      description:
        'Use this when the user explicitly provides an expense to log (amount, currency, category, date). Do not use for income or summaries.',
      inputSchema: {
        type: 'object',
        properties: {
          amount: { type: 'number', description: 'Expense amount in major units (e.g., 12.34).' },
          currency: { type: 'string', description: 'ISO 4217 currency code (e.g., USD, EUR).', minLength: 3, maxLength: 3 },
          category: { type: 'string', description: 'Category name (e.g., Groceries).' },
          merchant: { type: 'string', description: 'Merchant or payee (optional).' },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          type: { type: 'string', enum: ['expense', 'income'], description: 'Transaction type (WhatsApp parity).' },
          currency: { type: 'string', description: 'Optional currency filter (ISO code), e.g. USD.' },
          limit: { type: 'integer', minimum: 1, maximum: 200, description: 'Max number of rows (default 50).' },
          start_date: { type: 'string', description: 'Start date (YYYY-MM-DD), e.g. 2025-12-01.' },
          end_date: { type: 'string', description: 'End date (YYYY-MM-DD), e.g. 2025-12-31.' },
          household_id: { type: 'string', description: 'Optional household filter.' },
          household_name: { type: 'string', description: 'Optional household name (not used server-side).' },
          range: {
            type: 'object',
            properties: {
              startDate: { type: 'string', description: 'Start date (YYYY-MM-DD), e.g. 2025-12-01.' },
              endDate: { type: 'string', description: 'End date (YYYY-MM-DD), e.g. 2025-12-31.' },
            },
            additionalProperties: false,
          },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          currency: { type: 'string', minLength: 3, maxLength: 3, description: 'ISO currency code (3 letters), e.g., USD, EUR, GBP.' },
        },
        required: ['currency'],
        additionalProperties: false,
      },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          chart_type: { type: 'string', enum: ['bar', 'pie', 'donut', 'radar'], description: 'Chart type.' },
          labels: { type: 'array', items: { type: 'string' }, description: 'Labels (e.g., category names).' },
          data: { type: 'array', items: { type: 'number' }, description: 'Numeric values aligned to labels.' },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          scope: { type: 'string', description: 'Optional scope hint (e.g., “month”).' },
        },
        additionalProperties: false,
      },
      annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    ...
    },
    {
      ...
      description:
        'Use this when the user wants to add or remove a recurring transaction (e.g., “every month”, “cancel the subscription”).',
      inputSchema: {
        type: 'object',
        properties: {
          action: { type: 'string', enum: ['add', 'delete'], description: 'Choose add or delete.' },
          expense_id: { type: 'string', description: 'Required for delete (internal expense ID).' },
          amount: { type: 'number', description: 'Required for add.' },
          category: { type: 'string', description: 'Required for add.' },
          frequency: { type: 'string', enum: ['weekly', 'monthly', 'yearly'], description: 'Recurring frequency.' },
      ...
      inputSchema: {
        ...
        properties: {
          range: {
            type: 'object',
            properties: {
              startDate: { type: 'string', description: 'Start date (YYYY-MM-DD), e.g. 2025-12-01. Optional.' },
              endDate: { type: 'string', description: 'End date (YYYY-MM-DD), e.g. 2025-12-31. Optional.' },
            },
            additionalProperties: false,
          },
        },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          name: { type: 'string', description: 'New category name (e.g., Coffee).' },
        },
        required: ['name'],
        additionalProperties: false,
      },
    ...
    },
    {
      ...
      description: 'Use this when the user asks what categories exist or wants to pick a category. Read-only.',
      inputSchema: {
        type: 'object',
        properties: {},
        additionalProperties: false,
...
8) Privacy: Never show raw IDs (expense_id, household_id, user_id). Only show an ID if the user explicitly asks for it.
...
- Edit/remove a transaction: \`update_transaction\` / \`delete_transaction\`
...
`;
  ...
  server.setRequestHandler(
    ...
    async (request: CallToolRequest) => {
      ...
      const widgetMetaFor = (templateUri: string, extra?: Record<string, unknown>) => ({
      ...
      const authWidgetResult = (text: string) => {
        if (!supabaseConfig) {
          return {
            content: [{ type: 'text', text: 'Moneko sign-in is not configured on the server. Set SUPABASE_URL and SUPABASE_ANON_KEY.' }],
      ...
      const bearerUserId = tryGetSupabaseUserId(headerRecord);
      const storedSession = getAuthSession(headerRecord);
      const effectiveSession = bearerUserId
        ? { accessToken: getBearerToken(headerRecord) || '', userId: bearerUserId }
        : storedSession;

      const hasAuthHeader = Boolean(headerRecord.Authorization || headerRecord.authorization);
      const effectiveHeaders =
        bearerUserId && hasAuthHeader
          ? headerRecord
          : effectiveSession
            ? attachAuthHeader(headerRecord, effectiveSession.accessToken)
            : headerRecord;

      const withUserId = <T extends Record<string, unknown>>(payload: T): T & { userId?: string } => {
      ...
      if (!effectiveSession && !allowUnauthed.has(toolName)) {
        const prompt =
          ({
            ...
            get_summary: 'Sign in to your Moneko account to view your summary.',
            'moneko.get_budget': 'Sign in to your Moneko account to view your budget.',
            'moneko.set_budget': 'Sign in to your Moneko account to update your budget.',
            'moneko.save_expense': 'Sign in to your Moneko account to save expenses.',
            'moneko.save_income': 'Sign in to your Moneko account to save income.',
            'moneko.list_expenses': 'Sign in to your Moneko account to view expenses.',
            'moneko.expense_summary': 'Sign in to your Moneko account to view your summary.',
            'moneko.update_expense': 'Sign in to your Moneko account to update expenses.',
            'moneko.delete_expense': 'Sign in to your Moneko account to delete expenses.',
            'moneko.start_auth': 'Sign in to your Moneko account to continue.',
            'moneko.start_upgrade': 'Sign in to your Moneko account to continue.',
          } as Record<string, string>)[toolName] ?? 'Sign in to your Moneko account to continue.';
        return authWidgetResult(prompt);
      }

      const membershipWidgetResult = (text: string, snapshot?: { plan: string; status: string; currentPeriodEnd?: string | null; cancelAtPeriodEnd?: boolean }) => {
        return {
          content: [{ type: 'text', text }],
          structuredContent: {
            view: snapshot && hasActiveSubscription(snapshot.plan, snapshot.status) ? 'member' : 'paywall',
            message: text,
      ...
      const getSubscriptionSnapshot = async () => {
        ...
        try {
          payload = await proxyGet('/get-subscription', { userId: effectiveSession.userId }, effectiveHeaders, true);
        } catch {
        ...
        const plan = String(payload?.subscription?.plan ?? payload?.plan ?? 'free');
        const status = String(payload?.subscription?.status ?? payload?.status ?? 'none');
        const currentPeriodEnd =
          (payload?.subscription?.current_period_end ?? payload?.current_period_end ?? null) as string | null;
        const cancelAtPeriodEnd = Boolean(payload?.subscription?.cancel_at_period_end ?? payload?.cancel_at_period_end);
      ...
      if (
        ...
        return membershipWidgetResult('Start your free trial to unlock Moneko inside ChatGPT.', subscriptionSnapshot);
      }

      // Route to appropriate tool handler
      switch (toolName) {
        case 'moneko.sign_in': {
          return authWidgetResult('Please sign in to your Moneko account to continue.');
        }

        case 'moneko.open_app': {
          const tab = typeof (args as any)?.tab === 'string' ? String((args as any).tab) : 'overview';
          return {
            ...
            structuredContent: {
              ...
              subscription: subscriptionSnapshot
                ? {
                    ...
                    cancelAtPeriodEnd: Boolean(subscriptionSnapshot.cancelAtPeriodEnd),
                  }
                : null,
              supabaseUrl: supabaseConfig?.supabaseUrl,
              supabaseAnonKey: supabaseConfig?.supabaseAnonKey,
        ...
        case 'moneko.subscription_status': {
          if (!effectiveSession?.userId) {
            return authWidgetResult('Please sign in to check your membership status.');
          }
          if (!subscriptionSnapshot) {
            return membershipWidgetResult('Could not load membership status. Please try again.');
          }
          if (subscriptionSnapshot.subscribed) {
            return membershipWidgetResult('You have access to Moneko premium features.', subscriptionSnapshot);
          }
          return membershipWidgetResult('You are on the free plan. Start a free trial to unlock Moneko premium features.', subscriptionSnapshot);
        }

        case 'moneko.start_checkout': {
          const plan = String((args as any)?.plan || '').toLowerCase();
          const billingInterval = (args as any)?.billingInterval ? String((args as any).billingInterval).toLowerCase() : undefined;
          ...
          const payload = await proxy(
            ...
            plan === 'plus'
              ? { plan: 'plus', billingInterval: billingInterval === 'yearly' ? 'yearly' : 'monthly' }
              : { plan: 'lifetime' },
            effectiveHeaders,
            true
          );

          const url = payload?.url || payload?.checkoutUrl || payload?.checkout_url;
          if (!url) {
        ...
        case 'moneko.open_billing_portal': {
          if (!effectiveSession?.userId) {
            return authWidgetResult('Please sign in to manage your subscription.');
          }
          const returnUrl = typeof (args as any)?.returnUrl === 'string' ? String((args as any).returnUrl) : undefined;
          const { proxy } = await import('./lib/proxy.js');
          const payload = await proxy(
        ...
        case 'add_transaction': {
          ...
          const currency = (validArgs.currency ?? preferredCurrencyByConversation.get(conversationKey) ?? 'USD').toUpperCase();
          ...
          const recurrence_rule =
            validArgs.is_recurring
              ? { frequency: String(validArgs.frequency || 'MONTHLY').toUpperCase(), interval: 1, anchor_date: date }
              : undefined;
          ...
          const listPayload = await proxy('/list-expenses', withUserId({ limit: 50 }), effectiveHeaders, true);
          const props = toExpenseTablePayload(listPayload);
          return {
            content: [{ type: 'text', text: 'Transaction saved. Here are your recent expenses.' }],
        ...
        case 'update_transaction': {
          const { UpdateTransactionInput } = await import('./schemas.js');
          const { proxy } = await import('./lib/proxy.js');
          const { toExpenseTablePayload } = await import('./lib/transform.js');

          const validArgs = UpdateTransactionInput.parse(args);

          const updates: Record<string, unknown> = {};
          if (validArgs.amount !== undefined) updates.amount_cents = Math.round(validArgs.amount * 100);
          if (validArgs.category !== undefined) updates.category = validArgs.category;
          if (validArgs.description !== undefined) updates.raw_text = validArgs.description;
          if (validArgs.date !== undefined) updates.date = validArgs.date;
          if (validArgs.currency !== undefined) updates.currency = validArgs.currency;

          await proxy(
            '/update-expense',
            withUserId({
              expenseId: validArgs.expense_id,
              updates,
            }),
            effectiveHeaders,
            true
          ...
          const props = toExpenseTablePayload(listPayload);
          return {
            content: [{ type: 'text', text: 'Transaction updated.' }],
            structuredContent: props,
            _meta: widgetMetaFor(uris.expenseTable),
          };
        }
        ...
        case 'delete_transaction': {
          const { DeleteTransactionInput } = await import('./schemas.js');
          const { proxy } = await import('./lib/proxy.js');
          const { toExpenseTablePayload } = await import('./lib/transform.js');

          const validArgs = DeleteTransactionInput.parse(args);
          await proxy('/delete-expense', withUserId({ expenseId: validArgs.expense_id }), effectiveHeaders, true);

          const listPayload = await proxy('/list-expenses', withUserId({ limit: 50 }), effectiveHeaders, true);
          const props = toExpenseTablePayload(listPayload);
          return {
            content: [{ type: 'text', text: 'Transaction deleted.' }],
            structuredContent: props,
            _meta: widgetMetaFor(uris.expenseTable),
          };
        }
        ...
        case 'moneko.auth_status': {
          ...
          return {
            content: [{ type: 'text', text: authed ? 'You are signed in.' : 'You are not signed in.' }],
            structuredContent: {
        ...
        case 'moneko.clear_auth_session': {
          authSessionByConversation.delete(conversationKey);
          preferredCurrencyByConversation.delete(conversationKey);
          subscriptionByConversation.delete(conversationKey);
          return {
        ...
        case 'moneko.set_auth_session': {
          ...
          if (!jwtSecret) {
            throw new Error('Server is missing SUPABASE_JWT_SECRET and cannot verify sign-in.');
          }

          const verified = verifySupabaseJwtHs256(validArgs.access_token, jwtSecret);
          if (!verified) {
            throw new Error('Invalid session. Please sign in again.');
          }

          authSessionByConversation.set(conversationKey, {
          ...
          return {
            content: [{ type: 'text', text: 'Signed in. You can return to the chat and try again.' }],
        ...
        case 'log_expense': {
          ...
          const descriptionParts = [validArgs.merchant, validArgs.note].filter(Boolean).map((v) => String(v).trim()).filter(Boolean);
          ...
          return {
            content: [{ type: 'text', text: 'Logged that expense. Here are your most recent transactions.' }],
            structuredContent: props,
            _meta: widgetMetaFor(uris.expenseTable),
          };
        }
        ...
        case 'list_expenses': {
          const { ListExpensesMvpInput, ListTransactionsInput } = await import('./schemas.js');
          const { proxy } = await import('./lib/proxy.js');
          const { toCategoryBreakdownPayload, toExpenseTablePayload } = await import('./lib/transform.js');
          ...
          const parsed = (() => {
            ...
            if (parity.success) return { mode: 'parity' as const, value: parity.data };
            throw mvp.error;
          })();

          const type = parsed.mode === 'parity' ? parsed.value.type ?? 'expense' : 'expense';
          const limit = parsed.value.limit ?? 50;
          const startDate = parsed.mode === 'parity' ? parsed.value.start_date : parsed.value.range?.startDate;
          const endDate = parsed.mode === 'parity' ? parsed.value.end_date : parsed.value.range?.endDate;
          const currency = parsed.mode === 'parity' ? parsed.value.currency : undefined;
          ...
          if (type === 'expense' && hasWindow) {
            ...
            const sampleSize = (summaryPayload as any)?.data?.sampleSize ?? (summaryPayload as any)?.sampleSize;
          ...
          const payload =
            type === 'income'
              ? await proxy('/list-income', withUserId({ startDate, endDate, limit, currency }), effectiveHeaders, true)
              : await proxy('/list-expenses', withUserId({ startDate, endDate, limit, currency }), effectiveHeaders, true);
          const props = toExpenseTablePayload(payload);

          const categoryFilter = parsed.mode === 'mvp' ? parsed.value.category?.trim() : undefined;
          const filteredProps = categoryFilter
            ? { ...props, rows: props.rows.filter((r: any) => String(r.category || '').toLowerCase() === categoryFilter.toLowerCase()) }
            : props;
        ...
        case 'set_currency': {
          ...
          if (!effectiveSession?.userId) {
            return authWidgetResult('Sign in to update your preferred currency.');
          }
          ...
          preferredCurrencyByConversation.set(conversationKey, validArgs.currency.toUpperCase());

          return {
            content: [{ type: 'text', text: `Preferred currency updated to ${validArgs.currency}.` }],
          };
        }
        ...
        case 'financial_insight': {
          ...
          const { proxy } = await import('./lib/proxy.js');
          const { toBudgetStatusCard, toCategoryBreakdownPayload } = await import('./lib/transform.js');

          FinancialInsightInput.parse(args);

          if (!effectiveSession?.userId) {
            return authWidgetResult('Sign in to your Moneko account to view your financial snapshot.');
          }
          ...
          const [expenseSummary, incomeSummary, budgetStatus] = await Promise.all([
            proxy('/expenses-summary', withUserId({ startDate, endDate }), effectiveHeaders, true),
            proxy('/income-summary', withUserId({ startDate, endDate }), effectiveHeaders, true).catch((err) => ({
              success: false,
              error: err instanceof Error ? err.message : String(err),
            })),
            proxy('/get-budget', withUserId({ date: endDate }), effectiveHeaders, true).catch(() => null),
          ]);
          ...
          const breakdownList = Array.isArray((breakdownProps as any).breakdown)
            ? ((breakdownProps as any).breakdown as Array<{ totalAmountMajor?: unknown; totals?: unknown }>)
            : [];
          const totalSpent = breakdownList.reduce((sum: number, entry) => sum + (Number(entry.totalAmountMajor) || 0), 0);

          const incomeTotal =
            incomeSummary && (incomeSummary as any).success
              ? Number((incomeSummary as any).data?.totalIncome ?? (incomeSummary as any).data?.mtdIncome ?? 0)
              : null;
          ...
          const totals = Array.isArray(primary?.totals) ? (primary.totals as Array<any>) : [];
          const topCats = totals
            .slice()
            .sort((a: any, b: any) => (Number(b.share) || 0) - (Number(a.share) || 0))
            .slice(0, 5)
            .map((t: any) => `${String(t.category || 'Other')} ${(Number(t.share) * 100).toFixed(0)}%`)
            .join(', ');

          const budgetProps = budgetStatus ? toBudgetStatusCard(budgetStatus) : null;
          const budgetLine =
        ...
        case 'manage_recurring': {
          const { ManageRecurringInput } = await import('./schemas.js');
          const { proxy } = await import('./lib/proxy.js');

          const validArgs = ManageRecurringInput.parse(args);

          if (validArgs.action === 'delete') {
            if (!validArgs.expense_id) throw new Error('expense_id is required for delete');
            await proxy('/delete-expense', withUserId({ expenseId: validArgs.expense_id }), effectiveHeaders, true);
            return { content: [{ type: 'text', text: 'Recurring transaction deleted.' }] };
          }

          if (!validArgs.amount || !validArgs.category) {
            throw new Error('amount and category are required for recurring add');
          }

          const today = formatIsoDate(new Date());
          const currency = preferredCurrencyByConversation.get(conversationKey) ?? 'USD';
          const recurrence_rule = {
            frequency: String(validArgs.frequency || 'MONTHLY').toUpperCase(),
            interval: 1,
            anchor_date: today,
          };

          if ((validArgs.type || 'expense') === 'income') {
            await proxy(
              '/save-income',
              withUserId({
                amount: validArgs.amount,
            ...
            return { content: [{ type: 'text', text: 'Recurring income saved.' }] };
          }
          ...
          return { content: [{ type: 'text', text: 'Recurring expense saved.' }] };
        }
        ...
        case 'get_summary': {
          ...
          const startDate = validArgs.range?.startDate ?? formatIsoDate(new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1)));

          const payload = await proxy('/expenses-summary', withUserId({ startDate, endDate }), effectiveHeaders, true);
        ...
        case 'create_category': {
          ...
          return {
            content: [{ type: 'text', text: `Added category: ${normalizeCategoryName(validArgs.name)}` }],
        ...
        case 'moneko.get_budget': {
          ...
          const payload = await proxy('/get-budget', withUserId(validArgs as any), effectiveHeaders, true);
        ...
        case 'moneko.set_budget': {
          ...
          await proxy('/set-budget', withUserId(validArgs as any), effectiveHeaders, true);
        ...
        case 'moneko.save_expense': {
          ...
          await proxy('/save-expense', withUserId(validArgs as any), effectiveHeaders, true);
        ...
        case 'moneko.save_income': {
          ...
          await proxy('/save-income', withUserId(validArgs as any), effectiveHeaders, true);
        ...
        case 'moneko.list_expenses': {
          ...
          const payload = await proxy('/list-expenses', withUserId(validArgs as any), effectiveHeaders, true);
          const props = toExpenseTablePayload(payload);
        ...
        case 'moneko.expense_summary': {
          ...
          const payload = await proxy('/expenses-summary', withUserId(validArgs as any), effectiveHeaders, true);
          const props = toCategoryBreakdownPayload(payload);
        ...
        case 'moneko.update_expense': {
          ...
          await proxy(
            '/update-expense',
            withUserId({ expenseId: validArgs.expenseId, updates: validArgs.updates }),
            effectiveHeaders,
            true
          );
        ...
        case 'moneko.delete_expense': {
          ...
          await proxy('/delete-expense', withUserId(validArgs as any), effectiveHeaders, true);
        ...
        case 'moneko.start_auth': {
          ...
          const payload = await proxy('/start-auth', withUserId(validArgs as any), effectiveHeaders, true);
        ...
        case 'moneko.start_upgrade': {
          ...
          const payload = await proxy('/start-upgrade', withUserId(validArgs as any), effectiveHeaders, true);
